...
export default function AICoachScreen() {
  ...
  const loadLanguage = async () => {
    try {
      const saved = await AsyncStorage.getItem(SELECTED_LANGUAGE_KEY);
  ...
  const saveLanguage = async (lang) => {
    try {
      await AsyncStorage.setItem(SELECTED_LANGUAGE_KEY, lang);
  ...
  const loadCustomQuotes = async () => {
    try {
      const saved = await AsyncStorage.getItem(CUSTOM_QUOTES_KEY);
      if (saved) {
  ...
  const saveCustomQuotes = async (quotes) => {
    try {
      ...
      Alert.alert('Error', 'Could not save your quote');
    }
  };
  ...
  const loadData = async () => {
    try {
      ...
      if (playerData) {
        ...
        const generated = AICoachSystem?.generatePersonalizedTips?.(player.attrs || {});
        ...
        const db = await import('./utils/playerDatabase');
        const list = await db.getAllPlayers();
        setPlayers(list || []);
      ...
      console.error('AICoachScreen error:', err);
  ...
  const loadMotivationalQuotes = () => {
    ...
    if (filtered.length === 0 && selectedCategory !== 'all') {
      ...
      setTips([{
        ...
        tip: `No ${getCategoryLabel(selectedCategory)} quotes available yet.\n\nTry "All" or add your own!`,
        exercises: []
      }]);
      return;
    }
